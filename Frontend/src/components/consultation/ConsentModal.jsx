...
export default function ConsentModal({ open, onClose, onAccept, loading, appointmentType }) {
  ...
  const handleAccept = () => {
    ...
    onAccept({
      type: appointmentType === 'teleconsultation' ? 'teleconsultation' : 'consultation',
  ...
  return (
    <Modal open={open} onClose={onClose} title="Consentement patient" size="md"
      footer={
        <>
          <Button variant="outline" onClick={onClose}>Annuler</Button>
          <Button icon={Shield} onClick={handleAccept} disabled={!allAccepted} loading={loading}>
      ...
      <div className="space-y-4">
        <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-xl">
          ...
          <p className="text-sm text-blue-800">
            Conformément aux normes OMS et à la réglementation sur la télémédecine, votre consentement
            éclairé est requis avant toute consultation.
          </p>
        </div>
          ...
          {CONSENT_ITEMS.map((item, index) => (
            ...
            >
              <div className={`mt-0.5 w-5 h-5 rounded flex items-center justify-center flex-shrink-0 transition-colors ${
                accepted.includes(index) ? 'bg-primary-500' : 'border-2 border-gray-300'
              }`}>
                {accepted.includes(index) && <CheckCircle className="w-4 h-4 text-white" />}
              </div>
