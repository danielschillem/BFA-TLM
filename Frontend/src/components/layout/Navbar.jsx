...
export default function Navbar({ title }) {
  ...
  const { data: unread } = useQuery({
    ...
    queryFn: () => notificationsApi.unreadCount().then(r => r.data?.data?.unread_count ?? 0),
    refetchInterval: 30000,
  })

  const { data: notifData } = useQuery({
    queryKey: ['notifications-list'],
    queryFn: () => notificationsApi.list({ per_page: 8 }).then(r => ({
      items: r.data?.data ?? [],
  ...
  const handleLogout = async () => {
    try { const { authApi } = await import('@/api'); await authApi.logout() } catch {}
    logout()
    navigate('/login')
  }

  return (
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-gray-200/60 h-16 px-5 flex items-center justify-between">
      {/* Gauche */}
      <div className="flex items-center gap-3">
        <button onClick={toggleSidebar} className="p-2 rounded-xl hover:bg-gray-100 text-gray-500 transition-all duration-200 lg:hidden">
          <Menu className="w-5 h-5" />
        </button>
        <h1 className="text-base font-bold text-gray-800 hidden sm:block">{title}</h1>
      </div>
            ...
            <div className="absolute right-0 mt-2 w-80 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200/60 z-50 overflow-hidden animate-slide-up">
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {unread > 0 && (
                    <button onClick={() => markAllReadMutation.mutate()} className="text-xs text-primary-500 hover:underline flex items-center gap-1">
                      <CheckCheck className="w-3 h-3" /> Tout lire
                    </button>
                  )}
                  <button onClick={() => navigate('/messages')} className="text-xs text-primary-500 hover:underline">
                    Messages
                  </button>
                </div>
              </div>
                ...
                {notifData?.items?.length > 0 ? (
                  notifData.items.map(notif => (
                    <button key={notif.id}
                      ...
                      <p className="text-[11px] text-gray-400 mt-0.5">{fromNow(notif.created_at)}</p>
                    </button>
                  ))
                ) : (
                  <div className="py-6 text-center text-gray-400 text-[13px]">Aucune notification</div>
                )}
              </div>
            </div>
          )}
        ...
        <div className="relative">
          ...
          >
            <div className="w-8 h-8 bg-gradient-to-br from-primary-400 to-primary-600 rounded-xl flex items-center justify-center text-white font-bold text-xs shadow-md shadow-primary-500/20">
              {getInitials(`${user?.first_name ?? ''} ${user?.last_name ?? ''}`.trim() || user?.name || user?.full_name)}
            </div>
            <div className="hidden sm:block text-left">
              <p className="text-[13px] font-semibold text-gray-900 leading-none">{user?.first_name ?? user?.name ?? user?.full_name}</p>
              <p className="text-[11px] text-gray-500 capitalize">{user?.roles?.[0]?.replace('_', ' ')}</p>
            </div>
            ...
            <div className="absolute right-0 mt-2 w-48 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200/60 z-50 overflow-hidden animate-slide-up">
              <div className="py-1.5">
                <button onClick={() => { navigate('/profile'); setProfileOpen(false) }}
                  className="w-full text-left px-4 py-2 text-[13px] text-gray-700 hover:bg-gray-50 transition-colors rounded-lg mx-1" style={{width: 'calc(100% - 8px)'}}>
                  Mon profil
                </button>
                <hr className="my-1 border-gray-100 mx-3" />
                <button onClick={handleLogout}
                  className="w-full text-left px-4 py-2 text-[13px] text-red-600 hover:bg-red-50 transition-colors rounded-lg mx-1" style={{width: 'calc(100% - 8px)'}}>
                  Se déconnecter
                </button>
              </div>
            </div>
