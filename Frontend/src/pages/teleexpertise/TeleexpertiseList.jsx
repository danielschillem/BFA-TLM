...
import { Users, Plus, Clock, CheckCircle, XCircle, AlertCircle, ArrowRight } from 'lucide-react'
...
export default function TeleexpertiseList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => teleexpertiseApi.list(activeTab ? { status: activeTab } : {}).then(r => r.data.data?.data ?? []),
  })
  ...
  return (
    <AppLayout title="Téléexpertise">
      <div className="space-y-5 animate-fade-in">
        ...
        {/* Liste */}
        {isLoading ? <LoadingPage /> : requests.length === 0 ? (
          <EmptyState
            icon={Users}
            title="Aucune demande de téléexpertise"
            description="Les demandes de téléexpertise apparaîtront ici."
            action={isDoctor() ? (
              <Button onClick={() => navigate('/teleexpertise/new')} icon={Plus}>Nouvelle demande</Button>
            ) : undefined}
          />
        ) : (
          <div className="space-y-3">
            {requests.map(req => (
              <RequestCard key={req.id} request={req} onClick={() => navigate(`/teleexpertise/${req.id}`)} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="font-semibold text-gray-900 truncate">{req.title ?? req.reason}</p>
              <p className="text-xs text-gray-500 mt-0.5">
                {req.requesting_doctor?.last_name} → {req.specialist?.last_name ?? 'Spécialiste non assigné'}
              </p>
            </div>
          ...
          {req.clinical_summary && (
            <p className="mt-1.5 text-xs text-gray-500 line-clamp-2">{req.clinical_summary}</p>
          )}
        </div>
