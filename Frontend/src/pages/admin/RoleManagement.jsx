      ...
      {roles.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {roles.map((role) => {
            ...
            const gradient =
              ROLE_COLORS[role.name] || "from-gray-500 to-gray-600";
                ...
                <CardContent>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-9 h-9 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center text-white`}
                      >
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
                    <span className="flex items-center gap-1">
                      <Users className="w-3.5 h-3.5" /> {role.users_count}{" "}
                      utilisateur(s)
                    </span>
                    <span className="flex items-center gap-1">
                      <Key className="w-3.5 h-3.5" /> {role.permissions_count}{" "}
                      permission(s)
                    </span>
                  </div>
...
function RoleFormModal({ open, onClose, role, onSuccess }) {
  ...
  return (
    <Modal
      ...
      title={
        isEdit
          ? `Modifier : ${ROLE_LABELS[role.name] || role.name}`
          : "Nouveau rôle"
      }
      size="lg"
      footer={
        ...
        <div>
          ...
          {isProtectedRole && (
            <p className="text-xs text-amber-600 mt-1 flex items-center gap-1">
              <Lock className="w-3 h-3" /> Le nom des rôles système ne peut pas
              être modifié
            </p>
          )}
        </div>
...
function RoleDetailModal({ open, role, onClose }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`Rôle : ${ROLE_LABELS[role.name] || role.name}`}
      size="lg"
    >
      {isLoading ? (
        <div className="text-center py-8 text-gray-400">Chargement…</div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 font-mono">
                {role.name} — {data?.permissions?.length || 0} permissions
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {(data?.permissions || []).map((perm) => (
                <span
                  key={perm}
                  className="text-xs bg-primary-50 text-primary-700 px-2.5 py-1 rounded-lg font-mono"
                >
                  {perm}
                </span>
              ))}
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Vue complète des permissions par rôle
        </p>
      </div>
            ...
            <thead>
              <tr className="border-b border-gray-100">
                ...
                {matrixRoles.map((r) => (
                  <th
                    key={r.role}
                    className="px-3 py-3 text-center font-semibold text-gray-700 bg-gray-50/80 min-w-[100px]"
                  >
...
function UsersTab() {
  ...
  const { data: usersData, isLoading } = useQuery({
    ...
    queryFn: () =>
      adminApi
        .listUsers({ role: roleFilter, search, per_page: 50 })
    ...
    <>
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Modifiez les rôles attribués aux utilisateurs
          </p>
        </div>
              ...
              {users.map((u) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {(u.roles || []).map((role) => {
                      const gradient =
                        ROLE_COLORS[role] || "from-gray-500 to-gray-600";
                      return (
                        <span
                          key={role}
                          className={`text-[11px] text-white bg-gradient-to-r ${gradient} px-2 py-0.5 rounded-lg font-medium`}
                        >
...
function UserRoleModal({ open, user, onClose, onSuccess }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`Rôles de ${user.first_name} ${user.last_name}`}
      footer={
        <>
          <Button variant="secondary" onClick={onClose}>
            Annuler
          ...
          {allRoles.map((role) => {
            ...
            return (
              ...
              >
                ...
                />
                <div
                  className={`w-8 h-8 rounded-lg bg-gradient-to-br ${gradient} flex items-center justify-center text-white`}
                >
        ...
        <div className="flex flex-wrap gap-1.5">
          {selectedRoles.length === 0 ? (
            ...
            selectedRoles.map((r) => (
              <span
                key={r}
                className={`text-[11px] text-white bg-gradient-to-r ${ROLE_COLORS[r] || "from-gray-500 to-gray-600"} px-2.5 py-1 rounded-lg font-medium`}
              >
                {ROLE_LABELS[r] || r}
              </span>
            ))
