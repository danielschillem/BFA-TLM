...
import { Eye, EyeOff, Mail, Lock, Shield, Heart, Stethoscope } from 'lucide-react'
...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const body = res.data
      const requiresTwoFactor = body.requires_two_factor ?? body.data?.requires_two_factor
      const pendingUserId = body.data?.user?.id ?? body.user_id ?? body.data?.user_id
      ...
      const msg = err.response?.data?.message ?? err.message ?? 'Identifiants invalides.'
      toast.error(msg)
    } finally {
      setLoading(false)
    }
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="max-w-md">
            <h1 className="text-4xl font-extrabold text-white leading-tight mb-4">
              ...
              <span className="text-secondary-300">au service du Burkina Faso</span>
            </h1>
            <p className="text-white/70 text-base leading-relaxed">
              Accédez à la téléconsultation, la téléexpertise et le suivi médical 
              à distance. Une plateforme moderne pour rapprocher médecins et patients.
            </p>
          </div>
      ...
      <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-gray-50 via-white to-primary-50/30 p-6 lg:p-12 relative">
        ...
        <div className="w-full max-w-md relative z-10 animate-fade-in">
          ...
          <div className="lg:hidden text-center mb-8">
            <img src={liptakoIcon} alt="LiptakoCare" className="w-14 h-14 rounded-2xl object-cover shadow-lg shadow-primary-500/25 mb-4 mx-auto" />
            <h1 className="text-2xl font-bold text-gray-900">Plateforme TLM</h1>
            <p className="text-gray-500 text-sm mt-1">e-Santé & Télémédecine — Burkina Faso</p>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl shadow-gray-200/60 border border-white/60 p-8 lg:p-10">
            <div className="mb-8">
              ...
              <p className="text-gray-500 text-sm mt-1">Connectez-vous à votre espace santé</p>
            </div>
