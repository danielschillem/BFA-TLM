...
import {
  Bell, BellOff, Check, CheckCheck, Calendar, Video,
  MessageSquare, FileText, CreditCard, Stethoscope, AlertTriangle, Trash2
} from 'lucide-react'
...
function getNotifLink(notif) {
  ...
  if (data.consultation_id) return `/consultations/${data.consultation_id}/detail`
...
export default function NotificationCenter() {
  ...
  const { data: unreadCount } = useQuery({
    ...
    queryFn: () => notificationsApi.unreadCount().then(r => r.data.data?.count ?? r.data.count ?? r.data ?? 0),
  })
      ...
      <div className="space-y-5 animate-fade-in max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Centre de notifications</h2>
            {unreadCount > 0 && (
              <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{unreadCount}</span>
            )}
          </div>
          {unreadCount > 0 && (
            <Button variant="outline" size="sm" icon={CheckCheck} loading={markAllReadMutation.isPending}
        ...
        {/* List */}
        {isLoading ? <LoadingPage /> : notifications.length === 0 ? (
          <EmptyState
            icon={BellOff}
          ...
          <div className="space-y-1.5">
            {notifications.map(notif => {
              ...
              const title = notif.data?.title ?? notif.data?.message ?? notif.type?.split('\\')?.pop() ?? 'Notification'
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-3 p-3 rounded-xl transition-all cursor-pointer ${
                    isUnread ? 'bg-primary-50/50 border border-primary-100' : 'bg-white border border-gray-100 hover:bg-gray-50'
                  }`}
                >
