...
import Icd11Autocomplete from "@/components/ui/Icd11Autocomplete";
...
export default function ConsultationReport() {
  ...
  const shareMutation = useMutation({
    mutationFn: () =>
      consultationsApi.shareReport(id, { share_with: ["patient"] }),
    onSuccess: () => {
    ...
    <AppLayout title="Compte-rendu de consultation">
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                <FileText className="w-5 h-5 text-primary-600" /> Compte-rendu
                TLM_CR
              </h2>
              {consultation && (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {report?.signed_at ? (
                <span className="flex items-center gap-1.5 text-xs text-green-600 bg-green-50 border border-green-100 rounded-full px-3 py-1 font-medium">
                  <Check className="w-3.5 h-3.5" /> Signé le{" "}
                  ...
                  {diagList.map((d) => (
                    ...
                    >
                      <div>
                        ...
                        {d.icd_code && (
                          <span className="ml-2 text-xs text-cyan-700 bg-cyan-50 px-1.5 py-0.5 rounded font-mono">
                            {d.icd_code}
                          </span>
                        )}
                        {d.severity && (
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="font-semibold text-sm">
                      Prescriptions ({prescList.length})
                    </h4>
                  </div>
                </CardHeader>
                <CardContent className="space-y-2">
      ...
      >
        <p className="text-sm text-gray-600">
          En signant ce rapport, vous certifiez l'exactitude des informations
          médicales. La signature est électronique et horodatée.
        </p>
      </Modal>
      ...
      >
        <p className="text-sm text-gray-600">
          Le rapport signé sera partagé avec le patient via son espace
          personnel. Il recevra une notification par email.
        </p>
      </Modal>
...
function DiagnosticFormModal({ open, onClose, consultationId, onSuccess }) {
  ...
  const handleIcdSelect = (selected) => {
    setIcdValue(selected);
    if (selected) {
      setForm((f) => ({
        ...f,
        libelle: selected.title || f.libelle,
        code_cim: selected.code || "",
        code_icd11: selected.code || "",
        titre_icd11: selected.title || "",
        uri_icd11: selected.uri || "",
      }));
    } else {
      setForm((f) => ({
        ...f,
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="sm:col-span-2">
          <Icd11Autocomplete value={icdValue} onChange={handleIcdSelect} />
        </div>
        <div className="sm:col-span-2">
          <Input
            label="Intitul\u00e9 *"
        ...
        <Input
          label="Code CIM-11"
          value={form.code_icd11 || form.code_cim}
          readOnly
...
function ExamenFormModal({
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <Input
          label="Intitulé de l'examen *"
          value={form.libelle}
          onChange={(e) => set("libelle", e.target.value)}
          placeholder="Ex: NFS, Goutte épaisse, Radio thorax..."
        />
        <Input
          label="Type d'examen"
          value={form.type}
          onChange={(e) => set("type", e.target.value)}
          placeholder="Biologie, Imagerie, Fonctionnel..."
        />
        <Textarea
          label="Indication"
          ...
          <Button
            ...
            onClick={() => {
              if (!form.diagnostic_id) {
                toast.error("Veuillez associer un diagnostic");
                return;
              }
              mutation.mutate(form);
            }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {diagOptions.length > 0 ? (
          ...
          <div className="flex items-center text-sm text-amber-600">
            <AlertCircle className="w-4 h-4 mr-1" /> Ajoutez d'abord un
            diagnostic
          </div>
        )}
        <Input
...
function ExamenResultModal({ examen, onClose, onSuccess }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (form.date_reception_resultat)
      payload.date_reception_resultat = form.date_reception_resultat;
    ...
    >
      <div className="space-y-3">
        <Select
          ...
          value={form.statut}
          onChange={(e) => set("statut", e.target.value)}
          options={EXAMEN_STATUS_OPTIONS}
        />
        <Textarea
          label="Résultats"
          value={form.resultats}
          onChange={(e) => set("resultats", e.target.value)}
          rows={4}
        ...
        <Textarea
          ...
          rows={2}
          placeholder="Commentaire du praticien…"
        />
        <div className="grid grid-cols-2 gap-3">
          ...
          />
          <Input
            label="Date réception résultat"
            type="date"
            value={form.date_reception_resultat}
            onChange={(e) => set("date_reception_resultat", e.target.value)}
          />
        </div>
      </div>
    </Modal>
