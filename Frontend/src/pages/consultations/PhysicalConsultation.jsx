...
export default function PhysicalConsultation() {
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["consultation-physical", id] });
        ...
        <Card>
          <CardContent className="p-4">
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-sm">
              ...
              <div>
                <span className="text-gray-500 text-xs">Médecin</span>
              ...
              <div>
                ...
                <p className="font-semibold">{consultation.reason || "—"}</p>
              </div>
              <div>
            ...
            <Card>
              <CardHeader>
                <h3 className="font-semibold flex items-center gap-2">
                  <FileText className="w-5 h-5 text-emerald-600" /> Notes de
                  consultation
                </h3>
              </CardHeader>
              <CardContent className="space-y-4">
                <Textarea
                  ...
                  onChange={(e) => setNote("chief_complaint", e.target.value)}
                  placeholder="Motif principal de la visite…"
                  disabled={!isActive}
                />
                <Textarea
                  label="Anamnèse / Histoire de la maladie"
                  rows={3}
                  value={notes.history}
                  onChange={(e) => setNote("history", e.target.value)}
                  placeholder="Antécédents, symptômes, évolution…"
                  disabled={!isActive}
                />
                <Textarea
                  ...
                  onChange={(e) => setNote("examination", e.target.value)}
                  placeholder="Inspection, palpation, auscultation…"
                  disabled={!isActive}
                />
                <Textarea
                  label="Diagnostic"
                  ...
                  onChange={(e) => setNote("diagnosis", e.target.value)}
                ...
                <Textarea
                  ...
                  onChange={(e) => setNote("treatment_plan", e.target.value)}
                  placeholder="Traitement prescrit, conduite à tenir…"
                  disabled={!isActive}
                />
                <Textarea
                  ...
                  rows={2}
                  value={notes.notes}
                  onChange={(e) => setNote("notes", e.target.value)}
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setNote("follow_up_instructions", e.target.value)
                  }
                  placeholder="Rendez-vous de contrôle, surveillance…"
                  disabled={!isActive}
                />
              </CardContent>
            </Card>
          ...
          <div className="space-y-4">
            ...
            {examList.length > 0 && (
              <Card>
                <CardHeader>
                  <h3 className="font-semibold text-sm flex items-center gap-2">
                    <FlaskConical className="w-4 h-4 text-purple-600" /> Examens
                    ({examList.length})
                  </h3>
                </CardHeader>
                <CardContent className="space-y-2">
            ...
            {prescList.length > 0 && (
              <Card>
                <CardHeader>
                  <h3 className="font-semibold text-sm flex items-center gap-2">
                    <Pill className="w-4 h-4 text-indigo-600" /> Prescriptions (
                    {prescList.length})
                  </h3>
                </CardHeader>
                <CardContent className="space-y-2">
            ...
            {diagList.length === 0 &&
              ...
              prescList.length === 0 && (
                <EmptyState
                  icon={ClipboardList}
                  title="Aucune entité médicale"
                  description="Ajoutez des diagnostics, examens ou prescriptions."
                />
              )}
          </div>
        )}
        ...
        {activeSection === "vitals" && (
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <h3 className="font-semibold flex items-center gap-2">
                  <Activity className="w-5 h-5 text-emerald-600" /> Constantes
                  vitales
                </h3>
                {isActive && (
                  <Button
          ...
          <div className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl shadow-sm sticky bottom-4">
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                icon={FileText}
                onClick={() => navigate(`/consultations/${id}/report`)}
              >
                Rapport
              </Button>
              {!showEndConfirm ? (
...
function ExamenQuickModal({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onSuccess?.();
      onClose();
      setForm({ libelle: "", type: "", indication: "", urgent: false });
    },
    onError: () => toast.error("Erreur"),
  });
  return (
    ...
    >
      <div className="space-y-3">
        ...
        <Input
          ...
          placeholder="3x/jour pendant 5 jours"
        />
        <div className="grid grid-cols-2 gap-3">
          <Input
            label="Durée (jours)"
