...
import { Video, Clock, CheckCircle, AlertCircle, RefreshCw, User, Mic, Camera, Wifi, MapPin, Calendar, Shield } from 'lucide-react'
...
export default function WaitingRoom() {
  ...
  const [checks, setChecks] = useState({ camera: 'pending', mic: 'pending', network: 'pending' })
  ...
  const startMutation = useMutation({
    ...
    onError: err => toast.error(err.response?.data?.message ?? 'Erreur au démarrage'),
  })
  ...
  const consentMutation = useMutation({
    ...
    onError: (err) => toast.error(err.response?.data?.message ?? 'Erreur lors de l\'enregistrement du consentement'),
  })
  ...
  if (isLoading) return <AppLayout title="Salle d'attente"><LoadingPage /></AppLayout>
  if (!apt) return <AppLayout title="Salle d'attente"><EmptyState icon={AlertCircle} title="Rendez-vous introuvable" description="Impossible de charger les données du rendez-vous." /></AppLayout>

  return (
    <AppLayout title={isTeleconsultation ? "Salle d'attente virtuelle" : "Préparation consultation"}>
      <div className="max-w-xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className={`rounded-2xl p-8 text-white text-center ${
          ...
          <div className="relative inline-flex mb-4">
            <img src={logoImg} alt="LiptakoCare" className="w-20 h-20 rounded-full object-cover border-2 border-white/30" />
            {isTeleconsultation && (
          ...
          <h2 className="text-xl font-bold">
            {isTeleconsultation ? 'Salle d\'attente virtuelle' : 'Consultation physique'}
          </h2>
          ...
          <Card>
            <CardContent className="pt-4 space-y-3">
                  ...
                  {isDoctor() ? (
                    <>
                      <p className="font-medium text-gray-900">
                        {apt.patient?.full_name ?? `${apt.patient?.first_name ?? ''} ${apt.patient?.last_name ?? ''}`.trim()}
                      </p>
                    ...
                    <>
                      <p className="font-medium text-gray-900">
                        {apt.doctor?.full_name ?? `Dr. ${apt.doctor?.first_name ?? ''} ${apt.doctor?.last_name ?? ''}`.trim()}
                      </p>
              ...
              <div className="flex items-center gap-2 text-sm text-gray-500 pt-1 border-t border-gray-100">
                ...
                <span>{apt.date ? formatDateTime(`${apt.date}T${apt.time ?? '00:00'}`) : '—'}</span>
              ...
              {apt.reason && (
                <div className="text-sm text-gray-500 pt-1 border-t border-gray-100">
                  <span className="text-xs text-gray-400">Motif :</span> {apt.reason}
                </div>
              )}
            </CardContent>
          </Card>
        ...
        {isTeleconsultation && (
          <Card>
            <CardContent className="pt-4 space-y-3">
              <p className="text-sm font-semibold text-gray-700 mb-1">Vérification technique</p>
              {CHECKS.map(({ id: cid, label, icon: Icon }) => {
                ...
                return (
                  <div key={cid} className="flex items-center gap-3">
                    ...
                    {state === 'pending' && <RefreshCw className="w-4 h-4 text-gray-300 animate-spin" />}
                    {state === 'ok'      && <CheckCircle className="w-4 h-4 text-green-500" />}
                    {state === 'fail'    && <AlertCircle className="w-4 h-4 text-red-500" />}
                    {state === 'warn'    && <AlertCircle className="w-4 h-4 text-orange-400" />}
                  </div>
                )
              })}
              {!allChecksOk && (
                <p className="text-xs text-orange-600 bg-orange-50 rounded-lg p-2 mt-2">
                  Autorisez l'accès à la caméra et au microphone pour une meilleure expérience.
                </p>
              )}
            </CardContent>
          </Card>
        ...
        {!isTeleconsultation && (
          <Card>
            <CardContent className="pt-4 space-y-3">
              <p className="text-sm font-semibold text-gray-700 mb-1">Informations pratiques</p>
              <div className="space-y-2 text-sm text-gray-600">
                {apt?.structure?.name && (
                  <div className="flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-gray-400" />
                    <span>Structure : <span className="font-medium text-gray-800">{apt.structure.name}</span></span>
                  </div>
                )}
                <div className="flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-green-500" />
                  <span>Le patient doit se présenter au lieu de consultation</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {isDoctor() && (
          ...
          >
            {isTeleconsultation ? 'Démarrer la téléconsultation' : 'Démarrer la consultation'}
          </Button>
        )}
        {/* Patient attend */}
