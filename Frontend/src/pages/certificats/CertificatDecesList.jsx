...
import {
  ...
  ShieldCheck, Ban, AlertTriangle, BarChart3
} from 'lucide-react'
...
const STATUS_CONFIG = {
  brouillon: { label: 'Brouillon',  icon: Clock,        color: 'text-gray-700 bg-gray-50' },
  certifie:  { label: 'Certifié',   icon: CheckCircle,  color: 'text-blue-700 bg-blue-50' },
  valide:    { label: 'Validé',     icon: ShieldCheck,  color: 'text-green-700 bg-green-50' },
...
export default function CertificatDecesList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => certificatsDecesApi.list({
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => certificatsDecesApi.statistics().then(r => r.data.data ?? r.data ?? {}),
  })
    ...
    <AppLayout title="Certificats de décès">
      <div className="space-y-5 animate-fade-in">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <StatCard label="Total" value={stats?.total ?? certificats.length} icon={FileText} color="primary" />
          <StatCard label="Brouillons" value={stats?.brouillons ?? 0} icon={Clock} color="gray" />
          <StatCard label="Certifiés" value={stats?.certifies ?? 0} icon={CheckCircle} color="blue" />
          <StatCard label="Validés" value={stats?.valides ?? 0} icon={ShieldCheck} color="green" />
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button icon={Plus} onClick={() => navigate('/certificats-deces/nouveau')}>
            Nouveau certificat
          </Button>
          <Button variant="outline" icon={BarChart3} onClick={() => navigate('/certificats-deces/statistiques')}>
            Statistiques
          </Button>
        </div>
        ...
        {/* List */}
        {isLoading ? <LoadingPage /> : certificats.length === 0 ? (
          <EmptyState
            icon={FileText}
            title="Aucun certificat de décès"
            description="Créez un certificat de décès conforme au modèle OMS."
            action={<Button icon={Plus} onClick={() => navigate('/certificats-deces/nouveau')}>Créer</Button>}
          />
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-gray-400">
                      ...
                      {cert.cause_directe && (
                        <>
                          <span>·</span>
                          <span className="truncate max-w-[200px]">{cert.cause_directe}</span>
                        </>
                      )}
                    </div>
                  </div>
                  <span className={`flex items-center gap-1.5 text-xs font-medium rounded-full px-2.5 py-1 ${cfg.color}`}>
