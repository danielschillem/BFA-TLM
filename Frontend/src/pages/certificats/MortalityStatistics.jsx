...
export default function MortalityStatistics() {
  ...
  const stats = data ?? {};
  const totalDeaths = stats.total_deces ?? stats.total ?? 0;
  const topCauses =
    stats.top_causes ??
    stats.top_causes_icd11 ??
    stats.causes_principales ??
    [];
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              ...
              <StatCard
                label="Hommes"
                value={bySex.M ?? bySex.masculin ?? 0}
                icon={BarChart3}
                color="blue"
              />
              <StatCard
                label="Femmes"
                value={bySex.F ?? bySex.feminin ?? 0}
                icon={BarChart3}
                color="pink"
              />
              <StatCard
              ...
              <CardContent>
                {Array.isArray(topCauses) && topCauses.length > 0 ? (
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-sm text-gray-800">
                              {cause.cause ?? cause.label}
                            </span>
                            <span className="text-xs font-medium text-gray-500">
                              {cause.count ?? cause.nombre} (
                              {cause.percentage ?? cause.pourcentage}%)
                            </span>
                          </div>
                          <div className="w-full bg-gray-100 rounded-full h-1.5">
                            <div
                              ...
                              style={{
                                width: `${cause.percentage ?? cause.pourcentage ?? 0}%`,
                              }}
                            />
                          </div>
                        </div>
                  ...
                  <EmptyState
                    icon={BarChart3}
                    title="Aucune donnée"
                    description="Pas de statistiques disponibles pour cette période."
                  />
                )}
              </CardContent>
            </Card>
