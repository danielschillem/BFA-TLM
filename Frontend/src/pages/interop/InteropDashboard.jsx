    ...
    <Link to={linkTo} className="block">
      <Card hover className="p-4 transition-all hover:shadow-md">
        <div className="flex items-start justify-between mb-3">
          <div
            className={`w-10 h-10 rounded-lg flex items-center justify-center bg-${color}-50`}
          >
    ...
    <AppLayout title="Interopérabilité">
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard
            icon={FileJson}
            label="Ressources FHIR"
            value={fhirHealth.data?.rest?.[0]?.resource?.length ?? "—"}
            color="blue"
          />
          <StatCard icon={Shield} label="Standards" value="8" color="purple" />
          <StatCard
            icon={Activity}
            label="Protocoles DICOM"
            value="QIDO/WADO/STOW"
            color="teal"
          />
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <ServiceStatus
              ...
              description="CIM-11 · Recherche, validation, crosswalk ICD-10 vers ICD-11 · API WHO"
              linkTo="/interop/terminology"
            />
            <ServiceStatus
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-700">
                  <li className="flex items-center gap-2">
                    <FileJson className="w-4 h-4 text-orange-500" /> FHIR R4
                    (REST + JSON)
                  </li>
                  <li className="flex items-center gap-2">
                    <FileCode2 className="w-4 h-4 text-purple-500" /> CDA R2
                    (XML structuré)
                  </li>
                  <li className="flex items-center gap-2">
                    <MonitorDot className="w-4 h-4 text-red-500" /> DICOMweb
                    (imagerie)
                  </li>
                  <li className="flex items-center gap-2">
                    <Database className="w-4 h-4 text-teal-500" /> DHIS2
                    (agrégé)
                  </li>
                  <li className="flex items-center gap-2">
                    <Building2 className="w-4 h-4 text-amber-500" /> ENDOS
                    (national)
                  </li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-700">
                  <li className="flex items-center gap-2">
                    <BookOpen className="w-4 h-4 text-blue-500" /> SNOMED CT
                    (Snowstorm)
                  </li>
                  <li className="flex items-center gap-2">
                    <Globe className="w-4 h-4 text-green-500" /> ICD-11 / CIM-11
                    (OMS)
                  </li>
                  <li className="flex items-center gap-2">
                    <Pill className="w-4 h-4 text-purple-500" /> ATC
                    (médicaments)
                  </li>
                </ul>
              </div>
              <div>
                <h4 className="text-xs font-semibold text-gray-500 uppercase mb-2">
                  Codages complémentaires
                </h4>
                <ul className="space-y-2 text-sm text-gray-700">
                  <li className="flex items-center gap-2">
                    <Activity className="w-4 h-4 text-teal-500" /> LOINC
                    (observations)
                  </li>
                  <li className="flex items-center gap-2">
                    <Globe className="w-4 h-4 text-orange-500" /> ICD-10
                    (hérité)
                  </li>
                  <li className="flex items-center gap-2">
                    <Shield className="w-4 h-4 text-gray-500" /> OID 2.16.854.1
                    (BFA)
                  </li>
                </ul>
              </div>
