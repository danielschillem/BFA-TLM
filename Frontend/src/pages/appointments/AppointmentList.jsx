...
export default function AppointmentList() {
  ...
  const { data: rawData, isLoading } = useQuery({
    ...
    queryFn: () =>
      appointmentsApi.list({ page, per_page: perPage }).then((r) => r.data),
  });

  const appointments = rawData?.data?.data ?? rawData?.data ?? [];
  const pagination =
    rawData?.meta?.pagination ?? rawData?.data?.meta?.pagination ?? {};
  ...
  const handleExportCsv = () => {
    const rows = filtered.map((apt) => [
      apt.reason ?? "",
      (apt.actes ?? []).map((a) => a.libelle ?? a.name ?? "").join("; "),
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button
              ...
              onClick={() =>
                ...
                navigate(`/appointments/${selectedIds[0]}`)
              }
            >
              Éditer
            </Button>
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                <tbody>
                  {filtered.map((apt) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-800 uppercase">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t bg-gray-50 text-sm text-gray-500">
              <span>
                Affichage de {startIdx} à {endIdx} sur {totalEntries} entrées
              </span>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setPage(1)}
