...
export default function AppointmentDetail() {
  ...
  const cancelMutation = useMutation({
    ...
    onError: err => toast.error(err.response?.data?.message ?? 'Erreur lors de l\'annulation'),
  })
  ...
  const startConsultation = useMutation({
    ...
    onSuccess: (res) => {
      const consultationId = res.data?.data?.consultation?.id ?? res.data?.data?.id
      if (!consultationId) {
  ...
  const { data: doctorsData } = useQuery({
    ...
    queryFn: () => directoryApi.search({ search: delegateSearch, per_page: 10 }).then(r => r.data?.data ?? []),
    enabled: showDelegateModal && delegateSearch.length >= 2,
  })

  const delegateMutation = useMutation({
    mutationFn: () => appointmentsApi.delegate(id, { delegate_to: selectedDoctor.id, reason: delegateReason }),
    onSuccess: () => {
    ...
    onError: err => toast.error(err.response?.data?.message ?? 'Erreur lors de la délégation'),
  })

  if (isLoading) return <AppLayout title="Rendez-vous"><LoadingPage /></AppLayout>
  if (!apt)      return <AppLayout title="Rendez-vous"><div className="text-center py-12 text-gray-400">Introuvable</div></AppLayout>
  ...
  const canDelegate = isDoctor() && ['pending', 'confirmed'].includes(apt.status)
    ...
    <AppLayout title="Détail du rendez-vous">
      <div className="max-w-4xl mx-auto space-y-5 animate-fade-in">
          ...
          <CardContent className="py-4">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-gray-800">{apt.type === 'presentiel' ? '[PHYSIQUE] ' : '[EN LIGNE] '}{apt.reason ?? '—'}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-gray-800">{apt.time ?? '—'}</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="py-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-center flex-1">
                <p className="text-lg font-bold text-primary-700">Professionnel de santé &gt;&gt; {doctorName}</p>
                {apt.doctor?.matricule && <p className="text-sm text-gray-500">{apt.doctor.matricule}</p>}
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between text-sm px-3 py-1.5">
                ...
                <span className="font-semibold">{apt.doctor?.gender === 'male' ? 'Homme' : apt.doctor?.gender === 'female' ? 'Femme' : '—'}</span>
              </div>
              <div className="flex justify-between text-sm bg-gray-50 px-3 py-1.5 rounded">
                <span className="text-gray-600">Spécialité</span>
                <span className="font-semibold">{apt.doctor?.specialty ?? '—'}</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="py-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-center flex-1">
                <p className="text-lg font-bold text-primary-700">Patient &gt;&gt; {patientName}</p>
                {apt.patient?.identifiant && <p className="text-sm text-gray-600 font-semibold">{apt.patient.identifiant}</p>}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {/* Actes médicaux */}
          <Card>
            <CardHeader><h3 className="text-sm font-semibold text-gray-600">Actes médicaux</h3></CardHeader>
            <CardContent>
              {actes.length === 0 ? (
                <p className="text-sm text-gray-400 text-center py-4">Aucun acte associé</p>
              ) : (
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left py-2 px-2 font-bold text-gray-700" />
                      <th className="text-left py-2 px-2 font-bold text-gray-700">LIBELLE</th>
                      <th className="text-right py-2 px-2 font-bold text-gray-700">MONTANT</th>
                    </tr>
                  </thead>
                  <tbody>
                    {actes.map((acte, idx) => (
                      <tr key={acte.id} className="border-b border-gray-50">
                        ...
                        <td className="py-2 px-2 text-gray-800">{acte.libelle}</td>
                        <td className="py-2 px-2 text-right font-medium">{(acte.cout || 0).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <Card>
            <CardHeader><h3 className="text-sm font-semibold text-gray-600">Facturation & Paiement</h3></CardHeader>
            <CardContent>
              <div className="space-y-1">
                <div className="flex justify-between text-sm bg-gray-50 px-3 py-2 rounded">
                  <span className="text-gray-600">Type</span>
                  <span className="font-bold text-primary-700">Facturation</span>
                </div>
                ...
                <div className="flex justify-between text-sm bg-gray-50 px-3 py-2 rounded">
                  ...
                  <span className="font-bold text-primary-700">{totalActes.toLocaleString()} FCFA</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="bg-white border border-t-0 border-gray-200 rounded-b-xl p-4 flex items-center gap-3">
            <button
              ...
              <div className="text-left">
                <div className="text-[10px] uppercase tracking-wider opacity-80">En ligne</div>
            ...
            <button
              ...
              <Stethoscope className="w-5 h-5" />
              <div className="text-left">
                <div className="text-[10px] uppercase tracking-wider opacity-80">En présentiel</div>
        ...
        <div className="flex flex-wrap gap-3">
          {canStart && (
            <Button onClick={() => startConsultation.mutate()} loading={startConsultation.isPending}
              icon={apt?.type === 'presentiel' ? Stethoscope : Video} className="flex-1">
              {apt?.type === 'presentiel' ? 'Démarrer la consultation' : 'Démarrer la téléconsultation'}
            </Button>
          )}
          {canConfirm && (
            <Button onClick={() => confirmMutation.mutate()} loading={confirmMutation.isPending}
              variant="success" icon={CheckCircle} className="flex-1">
              Confirmer
            </Button>
          )}
          {canCancel && (
            <Button onClick={() => setShowCancelModal(true)} variant="danger" icon={X} className="flex-1">
              Annuler le RDV
            </Button>
          )}
          {canDelegate && (
            <Button onClick={() => setShowDelegateModal(true)} variant="outline" icon={UserPlus} className="flex-1">
              Déléguer
            </Button>
          )}
          {apt.status === 'completed' && apt.consultation?.id && (
            <Button onClick={() => navigate(`/consultations/${apt.consultation.id}/detail`)} variant="outline" icon={FileText} className="flex-1">
              Voir le compte-rendu
            </Button>
          )}
          {isPatient() && apt.status === 'confirmed' && apt.type !== 'presentiel' && (
            <Button onClick={() => navigate(`/appointments/${id}/waiting`)} variant="outline" icon={Clock} className="flex-1">
              Salle d'attente
            </Button>
          )}
          {apt.status === 'completed' && apt.consultation?.id && (
            <Button onClick={() => navigate(`/consultations/${apt.consultation.id}/payment`)} variant="outline" icon={CreditCard} className="flex-1">
              Paiement
            </Button>
          )}
        </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="danger" onClick={() => cancelMutation.mutate()} loading={cancelMutation.isPending} icon={X}>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Êtes-vous sûr de vouloir annuler ce rendez-vous ?</p>
          <div>
      ...
      <Modal
        ...
        onClose={() => { setShowDelegateModal(false); setSelectedDoctor(null); setDelegateSearch(''); setDelegateReason('') }}
        title="Déléguer le rendez-vous"
        footer={
          <>
            <Button variant="outline" onClick={() => setShowDelegateModal(false)}>Annuler</Button>
            <Button icon={UserPlus} onClick={() => delegateMutation.mutate()}
              loading={delegateMutation.isPending} disabled={!selectedDoctor}>
              Déléguer
            </Button>
          </>
        }
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Recherchez un professionnel de santé à qui déléguer ce rendez-vous.</p>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
            <input
              type="text"
              value={delegateSearch}
              onChange={e => { setDelegateSearch(e.target.value); setSelectedDoctor(null) }}
              ...
              {doctorsData.map(doc => (
                <button key={doc.id} onClick={() => setSelectedDoctor(doc)}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{doc.specialty ?? 'Médecin'}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          {selectedDoctor && (
            <div className="flex items-center gap-3 p-3 bg-primary-50 rounded-xl border border-primary-200">
              ...
              <div className="flex-1">
                <p className="font-medium text-gray-900">Dr. {selectedDoctor.first_name} {selectedDoctor.last_name}</p>
                <p className="text-xs text-gray-500">{selectedDoctor.specialty ?? 'Médecin'}</p>
              </div>
              <button onClick={() => setSelectedDoctor(null)} className="p-1 text-gray-400 hover:text-gray-600">
                <X className="w-4 h-4" />
              </button>
            </div>
          )}
