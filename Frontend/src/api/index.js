...
export const directoryApi = {
  ...
  getSlots:        (params) => apiClient.get('/directory/appointments/slots', { params }),
...
export const consultationsApi = {
  ...
  reportInterruption: (id, d)  => apiClient.post(`/consultations/${id}/interrupt`, d),
  rateVideoQuality:   (id, d)  => apiClient.post(`/consultations/${id}/rate-video`, d),
  transmitParams:     (id, d)  => apiClient.post(`/consultations/${id}/medical-parameters`, d),
  createReport:       (id, d)  => apiClient.post(`/consultations/${id}/report`, d),
  signReport:         (consultationId) => apiClient.post(`/consultations/${consultationId}/report/sign`, {}),
  shareReport:        (consultationId, payload) => apiClient.post(`/consultations/${consultationId}/report/share`, toSharePayload(payload)),
  downloadReportPdf:  (consultationId) => apiClient.get(`/consultations/${consultationId}/report/pdf`, { responseType: 'blob' }),
  downloadPrescriptionPdf: (consultationId) => apiClient.get(`/consultations/${consultationId}/prescription/pdf`, { responseType: 'blob' }),
...
export const documentsApi = {
  ...
  upload:   (data)   => apiClient.post('/documents', data, { headers: { 'Content-Type': 'multipart/form-data' } }),
...
export const paymentsApi = {
  initiate:       (cId, d) => apiClient.post(`/payments/consultations/${cId}/initiate`, d),
  confirm:        (data)   => apiClient.post('/payments/confirm', data),
  doctorValidate: (id)     => apiClient.post(`/payments/${id}/doctor-validate`, {}),
  downloadInvoice:(id)     => apiClient.get(`/payments/${id}/invoice`, { responseType: 'blob' }),
...
export const messagesApi = {
  ...
  conversation: (userId, p)      => apiClient.get(`/messages/conversation/${userId}`, { params: p }),
  send:         (userIdOrPayload, data)   => apiClient.post('/messages', toMessageSendPayload(userIdOrPayload, data)),
}
...
export const patientRecordApi = {
  ...
  verifyIdentity:  (pId, data) => apiClient.post(`/patients/${pId}/record/verify-identity`, data),
  archive:         (pId, data) => apiClient.post(`/patients/${pId}/record/archive`, data),
...
export const constantesApi = {
  listByDossier: (dossierId) => apiClient.get(`/dossiers/${dossierId}/constantes`),
}
...
export const consentsApi = {
  ...
  patientHistory: (patientId, params) => apiClient.get(`/consents/patient/${patientId}/history`, { params }),
}
...
export const gestionnaireApi = {
  ...
  listProfessionnels: (params)   => apiClient.get('/gestionnaire/professionnels', { params }),
  createProfessionnel:(data)     => apiClient.post('/gestionnaire/professionnels', data),
  getProfessionnel:   (id)       => apiClient.get(`/gestionnaire/professionnels/${id}`),
  updateProfessionnel:(id, data) => apiClient.put(`/gestionnaire/professionnels/${id}`, data),
  toggleStatus:       (id)       => apiClient.patch(`/gestionnaire/professionnels/${id}/toggle-status`),
...
export const adminApi = {
  ...
  listServices:        (structureId)        => apiClient.get(`/admin/structures/${structureId}/services`),
  createService:       (structureId, data)  => apiClient.post(`/admin/structures/${structureId}/services`, data),
  updateService:       (structureId, serviceId, data) => apiClient.put(`/admin/structures/${structureId}/services/${serviceId}`, data),
  deleteService:       (structureId, serviceId) => apiClient.delete(`/admin/structures/${structureId}/services/${serviceId}`),

  // Types de structures
  listTypeStructures:  (params)   => apiClient.get('/admin/type-structures', { params }),
  createTypeStructure: (data)     => apiClient.post('/admin/type-structures', data),
  updateTypeStructure: (id, data) => apiClient.put(`/admin/type-structures/${id}`, data),
  deleteTypeStructure: (id)       => apiClient.delete(`/admin/type-structures/${id}`),

  // Gestionnaires
  listGestionnaires:   (params)   => apiClient.get('/admin/gestionnaires', { params }),
  ...
  assignUserRoles:     (userId, data)  => apiClient.post(`/admin/users/${userId}/roles`, data),
}
...
export const fhirApi = {
  ...
  patientEverything:(id, params)=> apiClient.get(`/fhir/Patient/${id}/$everything`, { params }),
  searchPractitioner:(params)   => apiClient.get('/fhir/Practitioner', { params }),
  readPractitioner:(id)         => apiClient.get(`/fhir/Practitioner/${id}`),
  searchOrganization:(params)   => apiClient.get('/fhir/Organization', { params }),
  ...
  searchAllergyIntolerance:(p)  => apiClient.get('/fhir/AllergyIntolerance', { params: p }),
  searchMedicationRequest:(p)   => apiClient.get('/fhir/MedicationRequest', { params: p }),
  searchDiagnosticReport:(p)    => apiClient.get('/fhir/DiagnosticReport', { params: p }),
  searchAppointment:(params)    => apiClient.get('/fhir/Appointment', { params }),
  searchConsent:   (params)     => apiClient.get('/fhir/Consent', { params }),
  searchImagingStudy:(params)   => apiClient.get('/fhir/ImagingStudy', { params }),
}

// ── CDA R2 ────────────────────────────────────────────────────────────────────
export const cdaApi = {
  metadata:          ()   => apiClient.get('/cda/metadata'),
  patientCcd:        (id) => apiClient.get(`/cda/Patient/${id}/ccd`, { responseType: 'text', headers: { Accept: 'application/xml' } }),
  patientSummary:    (id) => apiClient.get(`/cda/Patient/${id}/summary`, { responseType: 'text', headers: { Accept: 'application/xml' } }),
  consultationNote:  (id) => apiClient.get(`/cda/Consultation/${id}/note`, { responseType: 'text', headers: { Accept: 'application/xml' } }),
  validate:          (xml)=> apiClient.post('/cda/validate', xml, { headers: { 'Content-Type': 'application/xml' } }),
}
...
export const certificatsDecesApi = {
  ...
  rejeter:      (id, data) => apiClient.post(`/certificats-deces/${id}/rejeter`, data),
  annuler:      (id, data) => apiClient.post(`/certificats-deces/${id}/annuler`, data),
  statistics:   (params) => apiClient.get('/certificats-deces/statistics/mortality', { params }),
}

// ── Terminologies (SNOMED CT + ATC) ───────────────────────────────────────────
export const terminologyApi = {
  // SNOMED CT
  snomedSearch:      (params)  => apiClient.get('/terminology/snomed/search', { params }),
  ...
  snomedDomainSearch:(domain, params) => apiClient.get(`/terminology/snomed/domain/${domain}`, { params }),
...
export const dicomApi = {
  ...
  upload:      (formData)=> apiClient.post('/dicom/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' }, timeout: 120000 }),
  sync:        (data)    => apiClient.post('/dicom/sync', data),
  series:      (uid)     => apiClient.get(`/dicom/studies/${uid}/series`),
  instances:   (studyUid, seriesUid) => apiClient.get(`/dicom/studies/${studyUid}/series/${seriesUid}/instances`),
  thumbnail:   (studyUid, seriesUid) => apiClient.get(`/dicom/studies/${studyUid}/series/${seriesUid}/thumbnail`, { responseType: 'blob' }),
}
...
export const dhis2Api = {
  ...
  organisationUnits:  (params) => apiClient.get('/dhis2/organisation-units', { params }),
