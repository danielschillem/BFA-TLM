...
const mockHandlers = {
  ...
  'GET /directory/doctors': async (params) => {
    ...
    return createMockPaginatedResponse(filtered, params?.page || 1, params?.per_page || 15)
  },
  ...
  'POST /appointments': async (data) => {
    ...
    const newAppointment = {
      ...
      ...data,
      status: 'pending',
      patient: mockAuthState.user ? { id: mockAuthState.user.id, full_name: mockAuthState.user.full_name } : mockAppointments[0].patient,
      doctor: mockDoctors.find(d => d.id === data.doctor_id) || mockAppointments[0].doctor,
      can_start: false,
      can_cancel: true,
      can_confirm: true,
  ...
  'POST /consultations/appointments/:appointmentId/start': async (_, appointmentId) => {
    await delay(MOCK_DELAY)
    const consultation = {
      id: Date.now(),
...
export const createMockClient = () => {
  const mockRequest = async (method, url, data = {}, config = {}) => {
    ...
    if (!result) {
      console.warn(`⚠️ [MOCK] No handler for ${method} ${url}`)
    ...
    try {
      ...
      const response = await result.handler(method === 'GET' ? allParams : data, ...result.params)
      console.log(`✅ [MOCK] ${method} ${url}`, response)
